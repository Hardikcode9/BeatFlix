...
const API_URL = `${API_BASE}/api/music`;
...
function Music() {
  const {
    ...
    togglePlay,
    cachedSongs: songs,
    setCachedSongs: setSongs,
    cachedSearch: search,
    setCachedSearch: setSearch,
    cachedActiveSearch: activeSearch,
    setCachedActiveSearch: setActiveSearch,
    cachedSelectedMood: selectedMood,
    setCachedSelectedMood: setSelectedMood,
  } = useMusic();
  ...
  useEffect(() => {
    if (location.state?.playSong) {
      playSong(location.state.playSong);
    }
  }, [location.state, playSong]);
  ...
  useEffect(() => {
    ...
    const handleUpdate = () => {
      setMyPlaylist(
        JSON.parse(localStorage.getItem("beatflix_playlist")) || []
      );
    };
    ...
    async (query = "Arijit Singh") => {
      try {
        ...
        if (!response.ok || !data.success) {
          throw new Error(data.message || "Music search failed.");
        }
        ...
        setSongs([]);
        setSearchError(error.message || "Unable to load music.");
      } finally {
        setLoading(false);
      }
    },
  ...
  }, [songs.length, activeSearch, searchMusic]);
  ...
  const handleSearch = (event) => {
    ...
    const query = value.trim() || "Arijit Singh";
  ...
  const handleMoodClick = (mood) => {
    ...
    const query =
      mood === "All" ? activeSearch || "Arijit Singh" : `${mood} mood songs`;
    searchMusic(query);
  };
      ...
      <section className="music-hero">
        ...
        <div className="hero-content">
          <div className="music-badge">
            ...
            <span>BEATFLIX AUDIO</span>
          </div>

          <h1 className="music-title">
            <span className="title-line">Feel Every</span>
            <span className="text-gradient">Frequency</span>
          </h1>

          <p className="music-subtitle">
            Discover songs and listen through the BeatFlix audio experience.
          </p>
        </div>
      </section>
      ...
      <div className="music-content-wrapper">
        <div className="music-search-container">
          <div className="search-glass-wrapper">
            <FaSearch className="music-search-icon" />
            <input
              type="text"
        ...
        <section className="music-grid-section">
          <div
            className="section-heading-row playlist-heading-clickable"
            onClick={() => navigate("/playlist")}
          >
            <div>
              <span className="section-eyebrow">YOUR COLLECTION</span>
              <h2>
                My Playlist <FaExternalLinkAlt className="playlist-link-icon" />
              </h2>
            </div>
          </div>
          {myPlaylist.length === 0 ? (
            <div className="music-playlist-empty">
              ...
              <p>Add songs from the search results below.</p>
            </div>
          ) : (
            <div className="music-grid">{myPlaylist.map(renderSongCard)}</div>
          )}
        </section>
        ...
        <section className="music-grid-section">
          ...
          {!loading && !searchError && songs.length === 0 && (
          ...
          {!loading && !searchError && songs.length > 0 && (
            <div className="music-grid">{songs.map(renderSongCard)}</div>
          )}
        </section>
      </div>
    </main>
