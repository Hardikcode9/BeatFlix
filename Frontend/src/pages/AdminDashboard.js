...
function AdminDashboard() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          fetch(`${API_BASE}/api/movies/top-rated`).then((r) => r.json()),
          fetch(`${API_BASE}/api/movies/new-releases`).then((r) => r.json()),
          fetch(`${API_BASE}/api/movies/top-india`).then((r) => r.json()),
          fetch(`${API_BASE}/api/movies/top-global`).then((r) => r.json()),
          fetch(`${API_BASE}/api/users/all`, { headers: authHeaders }).then(
            (r) => r.json()
          ),
        ]);
  ...
  const allMovies = useMemo(() => {
    const map = {
      ...
      "top-india": topIndiaMovies,
      "top-global": topGlobalMovies,
    };
  ...
  const totalMovieCount =
    ...
    newReleases.length +
    topIndiaMovies.length +
    topGlobalMovies.length;

  const filteredMovies = useMemo(() => {
    if (!movieSearch.trim()) return allMovies;
    return allMovies.filter((m) =>
      (m.title || m.name || "").toLowerCase().includes(movieSearch.toLowerCase())
    );
      ...
      <div className="section-block">
        ...
        <div className="tabs-simple">
          {[
            ...
            { key: "top-india", label: "India" },
            { key: "top-global", label: "Global" },
          ].map((cat) => (
            <button
              key={cat.key}
              className={`tab-simple ${
                movieCategory === cat.key ? "active" : ""
              }`}
              onClick={() => {
                setMovieCategory(cat.key);
                setMoviePage(1);
              }}
            >
              {cat.label}
                  ...
                  <tr key={movie.id}>
                    ...
                    <td>
                      <div className="tag-list">
                        {(movie.genre_ids || []).slice(0, 2).map((gid) => (
                          <span key={gid} className="tag-simple">
                            {GENRE_MAP[gid] || gid}
                          </span>
                        ))}
                      </div>
                    ...
                    <td>
                      <div className="flex-align-center gap-2 font-medium">
                        <FaStar className="star-icon text-xs" />{" "}
                        {movie.vote_average?.toFixed(1) || "N/A"}
                      </div>
                    </td>
                    <td className="text-right">
                      <div className="action-buttons">
                        <button className="btn-icon" aria-label="Edit movie">
                          <FaEdit />
                        </button>
                        <button
                  ...
                  <tr key={user._id}>
                    ...
                    <td>
                      <span className="plan-label">
                        {(user.subscription || "starter").toUpperCase()}
                      </span>
                    </td>
                    <td>
                      <span
                    ...
                    <td className="text-right">
                      <div className="action-buttons">
                        <button className="btn-icon" aria-label="Edit user">
                          <FaEdit />
                        </button>
                        <button
                          className="btn-icon danger"
      ...
      <div className="section-block max-w-3xl">
        ...
        <div className="settings-group">
          ...
          <div className="form-row">
            ...
            <input
              type="email"
              defaultValue="support@beatflix.com"
              className="input-simple"
            />
          </div>
        </div>
        ...
        <div className="settings-group mt-8">
          ...
          <div className="toggle-row">
            <div>
              ...
              <div className="toggle-desc">
                Allow signing in with Google accounts.
              </div>
            </div>
            <label className="switch">
              <input type="checkbox" defaultChecked />
